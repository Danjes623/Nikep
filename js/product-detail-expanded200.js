// Product Detail Page JavaScript - Expanded Version with Size Types and Favorites
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (currentProduct) {
    ...
    // ================================
    // NUEVA MODIFICACIÓN: INTEGRACIÓN CON FAVORITOS
    // Actualizar el estado del botón de favoritos
    // ================================
    const favoriteButton = document.querySelector('.favorite-button');
    if (favoriteButton && window.favoritesManager) {
      const isFavorite = window.favoritesManager.isFavorite(productId);
      favoriteButton.classList.toggle('active', isFavorite);
      
      // Actualizar texto del botón
      const favoriteText = favoriteButton.querySelector('span') || favoriteButton.childNodes[favoriteButton.childNodes.length - 1];
      if (favoriteText && favoriteText.nodeType === Node.TEXT_NODE) {
        favoriteText.textContent = isFavorite ? ' En favoritos' : ' Favorito';
      }
    }
  }
  ...
  // ================================
  // NUEVA MODIFICACIÓN: MANEJO DEL BOTÓN DE FAVORITOS
  // Agregar funcionalidad al botón de favoritos en la página de detalle
  // ================================
  document.addEventListener('click', function(e) {
    if (e.target.closest('.favorite-button')) {
      ...
      const isCurrentlyFavorite = window.favoritesManager.isFavorite(productId);
      ...
      if (isCurrentlyFavorite) {
        ...
        // Actualizar texto del botón
        const favoriteText = button.querySelector('span') || button.childNodes[button.childNodes.length - 1];
        if (favoriteText && favoriteText.nodeType === Node.TEXT_NODE) {
          favoriteText.textContent = ' Favorito';
        }
      } else {
        window.favoritesManager.addToFavorites(productId, currentProduct);
  ...
  // Esperar a que el sistema de favoritos esté disponible
  // ================================
  function initializeFavorites() {
    if (window.favoritesManager && currentProduct) {
      const favoriteButton = document.querySelector('.favorite-button');
      if (favoriteButton) {
        const isFavorite = window.favoritesManager.isFavorite(productId);
        favoriteButton.classList.toggle('active', isFavorite);
