...
function processCheckout(paymentMethod = 'standard') {
  if (cart.length === 0) {
    showNotification('El carrito está vacío');
    return;
  }

  // Mostrar mensaje de confirmación según el método de pago
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Handle checkout buttons - MODIFICADO para usar la nueva función
  ...
  if (checkoutButton) {
    checkoutButton.addEventListener('click', (e) => {
      ...
      processCheckout('standard'); // Llamar a la nueva función de checkout
    });
  }
  ...
  if (paypalButton) {
    paypalButton.addEventListener('click', (e) => {
      e.preventDefault(); // Prevenir comportamiento por defecto si es un enlace
      processCheckout('paypal'); // Llamar a la nueva función de checkout con PayPal
    });
  }
